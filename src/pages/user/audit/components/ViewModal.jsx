...
import { Tabs, Image, Row, Col, Modal, Descriptions, Button, Menu, Dropdown, message } from 'antd';
import { DownOutlined } from '@ant-design/icons';
import moment from 'moment';
import { auditAccept, auditRefuse } from '@/services/user';
import styles from '../index.less';

const { TabPane } = Tabs;

const ViewModal = (props) => {
  const { values, handleRefreshTable } = props;
  ...
  const reasons =
    props.settings.find((item) => item.key === 'option.auditFailureReason')?.val || [];

  const handleSelectReason = async ({ item, key }) => {
    try {
      const success = await auditRefuse({ id: values?.id, reason: key });
      if (success) {
        props.onCancel();
        handleRefreshTable();
      }
  ...
  const handleAuditAccept = async () => {
    try {
      const success = await auditAccept({ id: values?.id, reason: '' });
      if (success) {
        props.onCancel();
        handleRefreshTable();
      }
  ...
  const menu = (
    <Menu onClick={handleSelectReason}>
      {reasons.map((item) => (
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <Button type="primary" onClick={handleAuditAccept}>
            审核通过
          </Button>
        </>
      }
